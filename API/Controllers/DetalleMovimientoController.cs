...
public class DetalleMovimientoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<Pager<DetalleMovimientoDto>>> GetPaginacion([FromQuery] Params detalleMovimientoParams)
    {
        var entidad = await unitofwork.DetalleMovimientos.GetAllAsync(detalleMovimientoParams.PageIndex, detalleMovimientoParams.PageSize, detalleMovimientoParams.Search);
        var listEntidad = mapper.Map<List<DetalleMovimientoDto>>(entidad.registros);
        return new Pager<DetalleMovimientoDto>(listEntidad, entidad.totalRegistros, detalleMovimientoParams.PageIndex, detalleMovimientoParams.PageSize, detalleMovimientoParams.Search);
    }
